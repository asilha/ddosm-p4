...
def run_tshark(in_pcap, out_csv_gz):
    command = "tshark -r " + in_pcap + " -T fields -e ip.src -e ip.dst -e ip.id -e ip.checksum -e data.data | " + \
              "awk 'BEGIN {OFS=\",\"; print \"src\",\"dst\",\"src_delta\",\"dst_delta\",\"attack\" } {print $1,$2,$3,$4,substr($5,50,1) }' | " + \
              "gzip > " + out_csv_gz
    os.system(command)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Processes a PCAP file generated by ddosm-p4 and outputs a CSV.GZ file containing " + 
