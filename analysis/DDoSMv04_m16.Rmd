...
packets = read_pcap_csv(log2n, log2m, pcap_csv)
...
check_first_attack_ow(log2n, log2m, packets)
...
src_distinct = packets %>% filter(ow >= 194, ow <= 320) %>% group_by(ow, attack, src_delta) %>% summarize(srcs=n_distinct(src))
...
dst_distinct = packets %>% filter(ow >= 194, ow <= 320) %>% group_by(ow, attack, dst_delta) %>% summarize(dsts=n_distinct(dst))
...
stats_ci = function(packets, log2n, log2m) {
  ...
  tpr = packets %>% 
    ...
    group_by(ow) %>%
    summarize(n = n()) %>%
    summarize(mean = mean(n) / (0.2 * 65536), margin = qnorm(0.975) * sd(n)/sqrt(320-194+1) / (0.2 * 65536))
  
  fpr = packets %>% 
    filter(ow>=attack_first_ow, ow<=attack_last_ow, attack==FALSE, divert(src_delta, dst_delta)==TRUE) %>% 
    group_by(ow) %>%
    summarize(n = n()) %>%
    summarize(mean = mean(n) / (0.8 * 65536), margin = qnorm(0.975) * sd(n)/sqrt(320-194+1) / (0.8 * 65536))
