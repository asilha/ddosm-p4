...
summary_all_2 = summary_all %>% select(-log2n) %>% pivot_longer(p_tp:i95_fp_ub, names_to = "var") 

summary_all_2  

summary_all_2 %>% 
  filter(var %in% c("p_tp","p_fp")) %>%
  ggplot(aes(x=t, color=as.factor(log2m), shape=var)) + 
  geom_line(aes(y=value)) + geom_point(aes(y=value)) + 
  scale_color_manual(name="log2m", breaks=c(14,16,18), values=c("red","green","blue")) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(x="Mitigation Threshold (t)", y="") + 
  #facet_grid(cols = vars(as.factor(log2m))) +
  theme_classic()
