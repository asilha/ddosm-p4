...
ggsave(path="graphs", filename="entropy_all.svg")
...
ggsave(path="graphs", filename="entropy_attack.svg")
...
if (log2m == 14)  {
  ...
  cut = attack_first_ow + 8
} else if (log2m == 18) {
  cut = attack_first_ow + 2
} else {
  stop("Invalid log2m value.")
}

delta_plot_options = list(
  geom_point(mapping = aes(y=srcq1), color="blue4", size=dot_size),
  geom_point(mapping = aes(y=srcq2), color="yellow4", size=dot_size),
  geom_point(mapping = aes(y=srcq3), color="orangered4", size=dot_size))
...
ggsave(path="graphs", filename="src_deltas.svg")

delta_plot_options = list(
  geom_point(mapping = aes(y=dstq1), color="blue4", size=dot_size),
  geom_point(mapping = aes(y=dstq2), color="yellow4", size=dot_size),
  geom_point(mapping = aes(y=dstq3), color="orangered4", size=dot_size))
...
ggsave(path="graphs", filename="dst_deltas.svg")
...
ggsave(path="graphs", filename="src_delta_distribution.svg")
...
ggsave(path="graphs", filename="dst_delta_distribution.svg")
...
divert = function(src_delta, dst_delta) (src_delta >= src_threshold & dst_delta >= dst_threshold)
...
stats1 = get_stats(packets, log2n, log2m)
...
stats1 %>% 
  ...
  select(p_tp, i95_tp, p_fp, i95_fp)
...
ggsave(path="graphs", filename="false_negatives.svg")
graph_false_positives(packets)
ggsave(path="graphs", filename="false_positives.svg")
graph_results(packets)
ggsave(path="graphs", filename="results.svg")
...
I have an explanation for the growth of false positives. 
Results are not sensitive to window size. 
