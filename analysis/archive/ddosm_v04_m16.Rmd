...
src_distinct = packets %>% filter(ow >= attack_first_ow, ow <= attack_last_ow) %>% group_by(ow, attack, src_delta) %>% summarize(srcs=n_distinct(src))
...
dst_distinct = packets %>% filter(ow >= attack_first_ow, ow <= attack_last_ow) %>% group_by(ow, attack, dst_delta) %>% summarize(dsts=n_distinct(dst))
...
mitigation_out = packets %>% 
  filter(ow>=attack_first_ow, ow<=attack_last_ow) %>% 
  mutate(diverted=(src_delta>=src_threshold)) %>% 
