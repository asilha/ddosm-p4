...
src_distinct = packets %>% filter(ow >= 50, ow <= 80) %>% group_by(ow, attack, src_delta) %>% summarize(srcs=n_distinct(src))
...
dst_distinct = packets %>% filter(ow >= 50, ow <= 80) %>% group_by(ow, attack, dst_delta) %>% summarize(dsts=n_distinct(dst))
...
stats_ci = function(packets, log2n, log2m) {
  ...
  tpr = packets %>% 
    ...
    group_by(ow) %>%
    summarize(n = n()) %>%
    summarize(mean = mean(n) / (0.2 * 2^18), margin = qnorm(0.975) * sd(n)/sqrt(30) / (0.2 * 2^18))
  
  fpr = packets %>% 
    filter(ow>=attack_first_ow, ow<=attack_last_ow, attack==FALSE, divert(src_delta, dst_delta)==TRUE) %>% 
    group_by(ow) %>%
    summarize(n = n()) %>%
    summarize(mean = mean(n) / (0.8 * 2^18), margin = qnorm(0.975) * sd(n)/sqrt(30) / (0.8 * 2^18))
