...
# Based on https://github.com/skgrange/threadr/blob/master/R/twos_complement.R

#twos_complement <- function(x) 
#  purrr::map_dbl(x, twos_complement_worker)

#twos_complement_worker <- function(x) {
#  if (bitwAnd(x, bitwShiftL(1, 16 - 1)) != 0) x <- x - (bitwShiftL(1, 16))
#  return(x)
#}
...
## Delta Values (Source and Destination)

```{r}
packets = packets %>% rename(delta = src_delta)
packets = packets %>% mutate(delta=strtoi(delta), dst_delta=strtoi(dst_delta)) # Convert from hexadecimal to decimal
packets = packets %>% mutate(delta=twos_complement(delta), dst_delta=twos_complement(dst_delta)) # Convert from two's complement
```
