...
query %>% ggplot(mapping=aes(x=src_delta, y=sources, color=attack)) + geom_line() 
...
query %>% filter(attack==FALSE) %>% ggplot(mapping=aes(x=src_delta, y=sources)) + geom_line() 
# + coord_cartesian(xlim=c(0,1500),ylim=c(0,4000))

```

```{r}
query %>% filter(attack==TRUE) %>% ggplot(mapping=aes(x=src_delta, y=sources)) + geom_line() 
# + coord_cartesian(xlim=c(0,1500),ylim=c(0,4000))
