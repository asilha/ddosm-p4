...
class_evil = query %>% filter(src_delta>=threshold) %>% tally() 
class_good = query %>% filter(src_delta< threshold) %>% tally() 
message("Class evil: ", class_evil, " Class good: ", class_good, " Total: ", class_evil + class_good)

error_evil = query %>% filter(src_delta< threshold, attack==TRUE)  %>% tally() 
error_good = query %>% filter(src_delta>=threshold, attack==FALSE) %>% tally() 
...
query = packets %>% filter(ow>=first, ow<=last) %>% mutate(diverted=(src_delta>=threshold)) %>% group_by(ow)
...
query %>% filter(src_delta< threshold, attack==TRUE)  %>% summarize(n=n()) %>% 
  ggplot(mapping=aes(x=ow,y=n)) + geom_point() + ggtitle("False Negatives")

query %>% filter(src_delta>=threshold, attack==FALSE) %>% summarize(n=n()) %>% 
...
# Errors in classification. 
query %>% filter(src_delta< threshold, attack==TRUE)  %>% summarize(n=n()/101662) %>% 
  ggplot(mapping=aes(x=ow,y=n)) + geom_point() + ggtitle("False Negatives")

query %>% filter(src_delta>=threshold, attack==FALSE) %>% summarize(n=n()/406242) %>% 
  ggplot(mapping=aes(x=ow,y=n)) + geom_point() + ggtitle("False Positives")
