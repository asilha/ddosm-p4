...
packets %>% 
  ...
  group_by(ow) %>%
  summarize(n = n()) %>%
  summarize(meann = mean(n) / (0.2 * 16384), margin = qnorm(0.975) * sd(n)/sqrt(1280-770+1) / (0.2 * 16384))

packets %>% 
  filter(ow>=first, ow<=last, attack==FALSE, divert(src_delta, dst_delta)==TRUE) %>% 
  group_by(ow) %>%
  summarize(n = n()) %>%
  summarize(meann = mean(n) / (0.8 * 16384), margin = qnorm(0.975) * sd(n)/sqrt(1280-770+1) / (0.8 * 16384))
