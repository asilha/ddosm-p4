...
get_plot_tcad(tcad_m14, tcad_m14_k) + labs(title="Entropy for each Observation Window (m = 2^14)")
get_plot_tcad(tcad_m16, tcad_m16_k) + labs(title="Entropy for each Observation Window (m = 2^16)")
get_plot_tcad(tcad_m18, tcad_m18_k) + labs(title="Entropy for each Observation Window (m = 2^18)")
...
detection = function(log2n, log2m) (2^(log2n - log2m))     
training  = function(log2n, log2m) (2^(log2n - log2m - 1)) # Training length = detection / 2 
attack    = function(log2n, log2m) (2^(log2n - log2m - 1)) # Attack length   = detection / 2
safety    = function(log2n, log2m) (2^(log2n - log2m - 2)) # Pre-attack and post-attack = attack / 4 (each)

attack_first = function(log2n, log2m) (training(log2n, log2m) + safety(log2n, log2m) + 1)
attack_last  = function(log2n, log2m) (training(log2n, log2m) + safety(log2n, log2m) + attack(log2n, log2m))
...
get_plot_tcad_attack(tcad_m14, tcad_m14_k, log2n, 14) + labs(title="Entropy for each Observation Window - Attack Phase (m=2^14)") 
get_plot_tcad_attack(tcad_m16, tcad_m16_k, log2n, 16) + labs(title="Entropy for each Observation Window - Attack Phase (m=2^16)") 
get_plot_tcad_attack(tcad_m18, tcad_m18_k, log2n, 18) + labs(title="Entropy for each Observation Window - Attack Phase (m=2^18)") 
...
d = as.integer(2^log2n)

training_length = training(log2n,log2m)
detection_length = detection(log2n,log2m)

packets = packets %>% mutate(ow = get_ow(index,m) + training_length) # We add training_length because we have pre-initialized the training coefficients.
```
...
attack_first_packet = first * m + 1
attack_last_packet = last * m 
...
```{r dataset_checking_2}

packets %>% filter(index >= attack_first_packet - 4)
